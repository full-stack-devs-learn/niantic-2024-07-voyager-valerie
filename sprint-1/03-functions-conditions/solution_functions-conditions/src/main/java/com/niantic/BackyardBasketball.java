...
public class BackyardBasketball
{
    ...
    {
        int pointsPerShot = isThree ? 3 : 2;
        double tempBasketsNeeded = (double)desiredScore / pointsPerShot;
        double percent = shotPercentage / 100.0;

        double basketsNeeded = Math.ceil(tempBasketsNeeded);
        double tempShotsNeeded = basketsNeeded / percent;
        int shotsNeeded = (int)Math.ceil(tempShotsNeeded);
